...
import { ChevronLast, ChevronRight, LayoutDashboard, Users, ShoppingBag, Tag, Bookmark, LogOut, Layers, Package, User, FileText } from "lucide-react";
...
const navigationItems = [
  ...
  {
    to: "/dashboard/orders",
    icon: <Package size={20}></Package>,
    label: "Package",
  },
  {
    to: "/dashboard/invoices",
    icon: <FileText size={20}></FileText>,
    label: "Invoices",
  },
  {
    to: "/dashboard/banners",
    icon: <Layers size={20}></Layers>,
    label: "Banners",
  },
  {
    to: "/dashboard/products",
    icon: <ShoppingBag size={20}></ShoppingBag>,
    label: "Products",
  },
  {
    to: "/dashboard/categories",
    icon: <Tag size={20}></Tag>,
    label: "Categories",
  },
  {
    to: "/dashboard/brands",
    icon: <Bookmark size={20}></Bookmark>,
    label: "Brands",
  },
]
...
function NavItem({ to, icon, label, open, end, pathname }: NavItemProps) {
  return <NavLink to={to} end={end} className={cn("flex items-center p-2 rounded-xl text-sm font-medium hoverEffect gap-3 overflow-hidden text-white/80 hover:bg-gradient-to-r hover:from-slate-700/50 hover:to-slate-600/50 hover:text-white hover:shadow-lg hover:backdrop-blur-sm", pathname === to ? "bg-gradient-to-r from-[#29beb3]/20 to-[#a96bde]/20 text-white shadow-lg shadow-[#29beb3]/20 scale-105 right-1 ring-[#29beb3]/30 border border-white/10 backdrop-blur-sm" : "text-slate-300 hover:scale-102")}>
    <span>{icon}</span>
    {open && label}
  </NavLink>
}
...
const Sidebar = ({ open, setOpen }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1 flex-1 p-3 bg-gradient-to-b from-slate-800 to-slate-800/50">
        {
      ...
      <div className="p-4 border-t border-slate-600/50 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800">
        <motion.div
          ...
          className={cn("flex items-center gap-4 mb-4", open ? "justify-start" : "justify-center")}
        >
          <div className="h-10 w-10 rounded-full bg-gradient-to-br from-[#29beb3] to-[#a96bde] flex items-center justify-center text-white font-semibold overflow-hidden shadow-lg ring-2 ring-white/20">
            {user?.avatar ? <img src={user?.avatar} alt="userImage" className="h-full w-full object-cover"></img> : user?.name?.charAt(0).toUpperCase()}
          </div>
          <AnimatePresence>
            {open && (
              <motion.div
              ...
              >
                ...
                <p className="text-xs text-[#29beb3] capitalize font-medium">{user?.role}</p>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button variant={"outline"} size={open ? "default" : "icon"} onClick={logout} className="w-full border-red-500/30 hover:bg-red-600/20 hover:border-red-400/50 text-red-400 transition-colors bg-red-600/10 backdrop-blur-sm hover:text-white hoverEffect">
            <LogOut size={16} className={cn("mr-2", !open && "mr-0")}></LogOut> {open && "Logout"}
          </Button>
        </motion.div>
      </div>
    </motion.aside>
